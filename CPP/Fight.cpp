...
struct ItemsInfo {
	string name;
	string desc;
	int effect; //1 - защита, 2 - увеличение урона, 3 - восстановление (моментальное), 4 - регенераци€, 5 - неу€звимость, 6 - слепота, 7 - очищение, 8 - опустошение маны, 9 - моментальный ход
	int stacks;
	int cost;
};
...
void posEffects(InventPoke* cachedPok) {//√отова
...
void negEffects(int *enemy, InventPoke* cachedPok) {//√отова
...
void itemApply(int chosen, int *enemy, InventPoke* cachedPok) {//√отова
...
void inventory(int* lol, int *enemy, InventPoke* cachedPok) {//√отова
	cleanse();
